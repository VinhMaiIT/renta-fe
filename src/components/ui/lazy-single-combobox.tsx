    ...
    <div className={cn('w-full', className)}>
      ...
      >
        ...
        <div
          ref={anchorRef}
          className={cn(
            'group/trigger dark:bg-input/30 border-input relative flex w-full items-center overflow-hidden rounded-md border bg-transparent shadow-xs transition-[border-color,box-shadow]',
          ...
          >
            {hasValue && !disabled ? (
              showClearIcon && showArrowIcon ? (
                ...
                <button
                  type="button"
                  className="text-muted-foreground hover:text-foreground flex size-4 cursor-pointer items-center justify-center rounded"
                  onMouseDown={(e) => {
                    e.preventDefault();
                    e.stopPropagation();
                  }}
